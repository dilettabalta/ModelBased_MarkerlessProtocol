...
set(hTooltipControl, 'Visible', 'off');

legend_str = sprintf('Press DEL or BACKSPACE\nto undo last click\n\nPress ESC to exit calibration');
hLegend = text(1560,100,legend_str,'Fontsize',14);
...
%--------------------------------------------------------------------------
    function mouseMoveFcn(varargin)
        ...
        if C(1,1)>=0 && C(1,2)>=0 && C(1,1)<=1536 && C(1,2)<=1080
            str = {};
            switch length(x)
                case 0
                    str = {'Click on the ankle'};
                case 1
                    str = {'Click on the knee'};
                case 2
                    str = {'Click on the hip'};
                case 3
                    str = {'Click to confirm'};
            end
